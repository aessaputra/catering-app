<div>
    ...
    <div class="mb-8 text-center">
        <button wire:click="filterByCategory(null)"
                class="px-4 py-2 mr-2 mb-2 text-sm font-medium rounded-lg
                       {{ !$selectedCategoryId ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700' }}
                       hover:bg-blue-500 hover:text-white transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500">
            Semua Kategori
        </button>
        ...
        @foreach ($categories as $category)
            <button wire:click="filterByCategory({{ $category->id }})"
                    class="px-4 py-2 mr-2 mb-2 text-sm font-medium rounded-lg
                           {{ $selectedCategoryId == $category->id ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700' }}
                           hover:bg-blue-500 hover:text-white transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500">
                {{ $category->name }}
            </button>
        @endforeach
    ...
    <div wire:loading.class.delay="opacity-50" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                ...
                <div class="p-4">
                    ...
                    <div class="mt-4">
                        <button wire:click="addToCart({{ $product->id }})"
                            ...
                            <svg wire:loading wire:target="addToCart({{ $product->id }})" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                            </svg>
                            
                            {{-- Tampilkan ikon jika tidak loading --}}
                            <span wire:loading.remove wire:target="addToCart({{ $product->id }})"><i class="fas fa-cart-plus"></i></span>
